...
function CardGameCreative() {
    ...
    const addNewCard = () => {
      ...
      setTimeout(() => {
        setCards(prevCards => {
          const newCards = [...prevCards];
          newCards[newCards.length - 1] = React.cloneElement(newCard, { className: 'card' });
          return newCards;
        });
      }, 1);
    };
    ...
    return (
      ...
      <div className="flex flex-col items-center mt-12">
        <Navbar />
        <div
          className={`deck relative w-72 h-48 mt-40 ${isScattered ? 'is-scattered' : ''}`}
          onClick={e => {
            e.stopPropagation();
            removeTopCard();
          }}
...
export default CardGameCreative;